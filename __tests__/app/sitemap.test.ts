...
import { metadata as imageGalleryMetadata } from '../../src/app/image-gallery/page'
...
const metadataByRoute: Record<string, Metadata> = {
  ...
  '/image-gallery': imageGalleryMetadata,
  '/blog': blogMetadata,
...
describe('sitemap.xml generation', () => {
  ...
  it('should use the correct base URL', () => {
    ...
    for (const entry of result) {
      expect(entry.url).toContain('homesforchange.org')
    }
  })
  ...
  it('should include GitHub Pages base path in route URLs when configured', () => {
    ...
    expect(result.find((entry) => entry.url.endsWith('/FFC-EX-homesforchange.org/'))).toBeDefined()
    expect(
      result.find((entry) => entry.url.includes('/FFC-EX-homesforchange.org/privacy-policy'))
    ).toBeDefined()
  })
...
describe('sitemap URL shape matches the trailingSlash config', () => {
  ...
  it('emits every entry in the shape the export actually serves', () => {
    ...
    result.forEach((entry, index) => {
      ...
      expect(entry.url).toBe(`https://homesforchange.org${canonicalPath(routes[index].path)}`)
    })
  })
  ...
  it('keeps the root entry as the bare origin with a single slash', () => {
    ...
    expect(root.url).toBe('https://homesforchange.org/')
  ...
  it('applies the same shape under the GitHub Pages base path', () => {
    ...
    expect(urls[0]).toBe('https://homesforchange.org/FFC-EX-homesforchange.org/')
    expect(urls).toContain('https://homesforchange.org/FFC-EX-homesforchange.org/privacy-policy/')
